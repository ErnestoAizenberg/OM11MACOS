...
def configure_browser_api(
    ...
    def start_browser(ws_url: str, user_id: str) -> None:
        ...
        try:
            ...
            logger.debug(f"Request details - URL: {url}, Params: {params}", exc_info=True)
    ...
    def get_profiles() -> Any:
        ...
        try:
            logger.info(f"Fetching profiles from {api_url} for browser type {browser_type}")
            ...
            logger.debug(f"Profile data sample: {profiles[:1] if profiles else 'No profiles'}")  # Log first profile for sample
            ...
            logger.debug(f"Request details - URL: {api_url}, Type: {browser_type}", exc_info=True)
            return jsonify(
                success=False,
                error=str(e),
                details={
    ...
    def start_profile() -> Any:
        ...
        try:
            ...
            logger.info(f"Profile started successfully, connecting to WebSocket")
            logger.debug(f"WebSocket URL: {ws_url}")
            
            start_browser(ws_url=ws_url, user_id=user_id)
            
            logger.info(f"Browser profile {profile_id} fully initialized for user {user_id}")
            return jsonify(
                success=True,
                ws_url=ws_url,
                details={
            ...
            logger.debug(f"Error details for ValueError", exc_info=True)
            return jsonify(
                success=False,
                error=str(e),
                details={
            ...
            logger.debug(f"Request details for failed operation", exc_info=True)
            return jsonify(
                success=False,
                error=str(e),
                details={
    ...
    def browser_status() -> Any:
        ...
        try:
            ...
            logger.info(f"Status check complete - connected: {connected}, profiles: {active_profiles}")
    ...
    def disconnect_all() -> Any:
        ...
        try:
            ...
            if post_disconnect_profiles:
                logger.warning(f"Disconnect incomplete - remaining profiles: {len(post_disconnect_profiles)}")
            else:
