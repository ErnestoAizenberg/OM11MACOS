...
from flask import Flask, Response, abort, jsonify, request, session, make_response

from app.agent.agent_manager_sq import AgentManager
from app.agent.manus_client import ManusClient
from app.telegram.api import TelegramClient
...
def configure_agent_api(
        ...
        try:
            ...
            if not user_is_connected:
                return make_response(
                    jsonify({"success": False, "error": "Connected browser is not found, connect a browser first"}),
                    400,
                )
    ...
    @login_required
    def get_command_history() -> Response:
    ...
    @login_required
    def start_agent() -> Response:
        user_id: str = session.get("user_id", '')
        try:
            pid: Optional[int] = agent_manager.start_agent(user_id)
            if not pid:
                return make_response(
    ...
    @login_required
    def stop_agent() -> Response:
        user_id: str = session.get("user_id", '')
        try:
            success: bool = agent_manager.stop_agent(user_id)
            if not success:
                return make_response(
                    jsonify({"success": False, "error": "Agent not running"}),
                    400,
                )
